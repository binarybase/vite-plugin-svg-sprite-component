...
function createPlugin(options) {
    ...
    var symbolId = options.symbolId, component = options.component, removeAttrs = options.removeAttrs, transform = options.transform, matcher = options.matcher;
    var cache = new Map();
    return {
        ...
        transform: function (source, path) { return __awaiter(_this, void 0, void 0, function () {
            ...
            return __generator(this, function (_b) {
                ...
                code = (0, fs_1.readFileSync)(path).toString();
                svgName = (0, path_1.basename)(path, '.svg');
                finalSymbolId = symbolId ? symbolId(svgName, path) : svgName;
                svgCode = code.match(/<svg[^]*<\/svg>/);
                if (!svgCode)
                    return [2 /*return*/, source];
                svgNode = (0, htmlparser2_1.parseDOM)(svgCode[0], { lowerCaseAttributeNames: false })[0];
                svgNode.name = 'symbol';
                svgNode.attribs.id = finalSymbolId;
                if (!Reflect.has(svgNode.attribs, 'viewBox')) {
                    _a = svgNode.attribs, width = _a.width, height = _a.height;
                    svgNode.attribs.viewBox = "0 0 ".concat(width || 200, " ").concat(height || 200);
                }
                ...
                childElements.map(function (element) {
                    ...
                    if (hasStyle) {
                        ...
                        for (var i in styleObj) {
                            styleStr += "".concat(i, ":").concat(styleObj[i], ";");
                        }
                        if (styleStr.trim()) {
                ...
                if (component) {
                    ...
                    if (component.type === 'vue') {
                        componentCode = "\n        import {h} from \"vue\";\n        export const ".concat(componentName, " = (props)=> h('svg',props,h('use',{'xlink:href':'#").concat(finalSymbolId, "'}));\n        ").concat(defaultExport ? "export default ".concat(componentName) : source, ";\n        ");
                    }
                }
